...
const handleInertiaBefore = (event) => {
    if (form.processing) {
        if (
            !confirm(
                'The form is loading. Are you sure you want to leave this page?',
            )
        ) {
            event.preventDefault();
        }
...
const submitGallery = () => {
    form.post(
        route(
            form.id ? 'profile.galleries.update' : 'profile.galleries.store',
            form.id,
        ),
        {
        ...
        <div>
            <form @submit.prevent="submitGallery" class="px-4 md:px-0">
                <SuccessMessages info class="mt-4">
                    <p class="p-0 font-bold text-yellow">
                        Only concert photos can be published on this page.
                    </p>
                    Event photos may also be featured on our
                    <b
                        ><a
                            target="_blank"
                            href="https://www.instagram.com/rocker._.am/"
                            >Instagram page</a
                        ></b
                    >
                    if they cover the entire event. Photo credits are always
                    included, and image quality is never reduced or cropped.
                </SuccessMessages>
                <ErrorMessages :messages="$page.props.errors" />
                <div class="mt-4 flex flex-col-reverse gap-4 md:flex-row">
                    ...
                    <div class="flex w-full flex-col gap-y-2 md:w-1/2">
                        ...
                        <Multiselect
                            v-model="form.bands"
                            tooltip="If you tag the bands, the photo album will also appear on their page"
                            :options="bandsList"
                            text="Bands"
                            multiple
                        />
                ...
                <ProgressBar
                    ...
                    class="mb-5 mt-10 w-full bg-green"
                ...
                <div v-if="form.processing" class="text-red-500 mt-2">
                    Uploading...
                </div>
            </form>
        </div>
