...
const lang = ref(getUrlQuery('lang') ?? props.blog.title['en'] ? 'en' : props.blog.title['am'] ? 'am' : 'ru');
...
<template>
  <GuestLayout
    :meta="{title:  blog.title[lang]  ,image:blog.cover.large , description: blog.description['en'] ?? blog.description['am'] , author: blog.author ?? blog.user.name,keywords: blog.bands.map(band => band?.name).join(',')}">
        ...
        >
          <div class="flex justify-between gap-2 absolute bottom-0 w-full bg-black bg-opacity-20 p-4">
            <div class="flex gap-2">
              ...
              <button v-if="blog.title['ru']" @click="setLang('ru')">RUS</button>
            </div>
            <SocialShare class="absolute right-0 bottom-0 bg-opacity-20 bg-graydark"
                         :title="blog.title['en'] ?? blog.title['am']  ?? blog.title['ru'] "
                         :url />
          </div>
        </div>
      </div>
