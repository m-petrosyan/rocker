<script setup>
import { router } from '@inertiajs/vue3';
import NavLink from '@/Components/NavLink.vue';
import DeleteIcon from '@/Components/Icons/DeleteIcon.vue';
import EditIcon from '@/Components/Icons/EditIcon.vue';
import EyesIcon from '@/Components/Icons/EyesIcon.vue';
import { formatDateTime } from '@/Helpers/dateFormatHelper.js';
import { isSSR } from '@/Helpers/ssrHelper.js';
import AddCard from '@/Components/Cards/AddCard.vue';

defineProps({
    galleries: {
        type: Object,
        required: true,
    },
    title: {
        type: String,
        default: '',
    },
    user: {
        type: Boolean,
        default: false,
    },
    owner: {
        type: Boolean,
        default: false,
    },
    isAdmin: {
        type: Boolean,
        default: false,
    },
    add: {
        type: Boolean,
        default: false,
    },
    more: {
        type: Boolean,
        default: false,
    },
    profile: {
        type: Boolean,
        default: false,
    },
});

const deleteGallery = (id) => {
    if (confirm('Are you sure you want to delete this gallery?')) {
        router.delete(route('profile.galleries.destroy', id), {
            preserveState: false,
            preserveScroll: true,
        });
    }
};
</script>

<template>
    <div class="mt-10">
        <h3 v-if="title" class="text-center">{{ title }}</h3>
        <div class="mt-10 grid gap-y-10 md:grid-cols-2 lg:grid-cols-4">
            <NavLink
                v-for="gallery in galleries"
                :href="route('galleries.show', gallery.id)"
                :key="gallery.id"
                class="flex flex-col items-center p-4"
            >
                <div class="relative h-64 w-full bg-black">
                    <div
                        class="h-full w-full overflow-hidden rounded-lg rounded-xl bg-black"
                    >
                        <img
                            :src="gallery.cover_img.thumb"
                            class="h-full w-full object-cover"
                            loading="lazy"
                            :alt="gallery.title"
                            @error="
                                $event.target.src = gallery.cover_img.original
                            "
                        />
                    </div>
                    <div
                        v-if="isAdmin"
                        class="absolute left-0 top-0 bg-red px-1"
                    >
                        {{ gallery.total_mb }} MB
                    </div>
                    <div class="absolute bottom-8 right-0 bg-orange px-1">
                        {{ formatDateTime(gallery.date, 'DD.MM.YY') }}
                    </div>
                    <a
                        v-if="gallery.user"
                        :href="route('profile.show', gallery.user.username)"
                        class="absolute bottom-0 left-0 w-full bg-blackTransparent2 p-1 text-center"
                        @click.stop
                    >
                        by {{ gallery.user.name }}
                    </a>
                    <div
                        v-if="(owner || isAdmin) && profile"
                        class="absolute bottom-0 flex h-full w-full flex-col justify-between bg-blackTransparent2 p-1"
                    >
                        <div class="flex justify-end gap-y-2">
                            <div
                                @click.prevent
                                tooltip="Views"
                                class="flex cursor-default items-center gap-x-2"
                            >
                                <EyesIcon />
                                <b>{{ gallery.views_count }}</b>
                            </div>
                        </div>

                        <div class="flex justify-between">
                            <NavLink
                                tooltip="Edit"
                                :href="
                                    route('profile.galleries.edit', gallery.id)
                                "
                            >
                                <EditIcon />
                            </NavLink>
                            <button
                                tooltip="Delete"
                                @click.prevent="deleteGallery(gallery.id)"
                                class="text-red-500 hover:text-red-700"
                            >
                                <DeleteIcon />
                            </button>
                        </div>
                    </div>
                </div>
                <div class="p-2">
                    <p v-if="isSSR">
                        {{ gallery.title }} by {{ gallery.user?.name }}
                    </p>
                    <p v-else class="text-lg font-semibold">
                        {{ gallery.title }}
                    </p>
                </div>
            </NavLink>
            <AddCard
                v-if="add"
                title="Add gallery"
                :href="route('profile.galleries.create')"
                key="add-gallery"
            />
        </div>
        <div v-if="more" class="col-span-full py-4 text-center">
            <NavLink
                :href="route('galleries.index')"
                label="Galleries list"
                class="font-bold text-orange"
            >
                Browse more photo galleries
            </NavLink>
        </div>
    </div>
</template>
