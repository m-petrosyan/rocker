...
<template>
  ...
  >
    ...
    <div v-if="show" class="mb-8 relative py-4">
      <div class="flex flex-col-reverse flex-col md:flex-row justify-center gap-8 items-start">
        <div class="space-y-3 w-full md:w-auto text-center">
          <label class="block text-[9px] font-bold text-gray/40 uppercase tracking-[0.2em] ml-2">Select Date
            Range</label>
          <div class="p-2">
            <RangeDatePicker v-model="dateRange" />
          </div>
        </div>
        <div v-if="auth?.user?.settings?.country === 'all'"
             class="flex w-full md:w-auto flex-col justify-between h-full space-y-6">
          <div class="space-y-3">
            <label class="block text-[9px] font-bold text-gray/40 uppercase tracking-[0.2em] ml-2 text-center">Filter by
              Country</label>
            <div class="flex justify-center gap-x-4 ml-2">
              <button
